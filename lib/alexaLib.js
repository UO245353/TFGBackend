...
module.exports = app => {
  ...
  function getDataToSaveFromMachine(machine){
    ...
    return {
      ...
      themeNumber: machine.getThemeNumber(),
      responseNumber: machine.getResponseNumber()
    };
  }

  function getMachineFromSessionData(data){
    console.log(data);
    let machine = new stateMachine(data.userId);

    machine.goto(data.state);

    machine.setThemeNumber(data.themeNumber);
    machine.setResponseNumber(data.responseNumber);
  ...
  const stateMachine = require('javascript-state-machine').factory({
    ...
    methods: {
      setThemeNumber: function(newThemeNumber){
        this.themeNumber = newThemeNumber;

        return true;
      },
      setResponseNumber: function(newResponseNumber){
        this.responseNumber = newResponseNumber;
      ...
      getThemeNumber: function() {

        return this.themeNumber;
      },
      getResponseNumber: function() {
  ...
  const handlers = {
    ...
    LeeTema: {
      ...
      handle(handlerInput) {
        ...
        .then(machine => machine.dictate(themeNumber))
        .then(result => {
          mensaje = result;
