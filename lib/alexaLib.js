  ...
  const handlers = {
    LaunchRequestHandler: {
      ...
      handle(handlerInput) {
        ...
        return Promise.resolve()
        .then(() => handlerInput.attributesManager.setSessionAttributes({stateMachine: new stateMachine(handlerInput.requestEnvelope.session.sessionId)}) )
    ...
    LeeTema: {
      ...
      handle(handlerInput) {
        ...
        return Promise.resolve()
        .then(() => {
          stateMachine = require('circular-json-es6').parse(handlerInput.attributesManager.getSessionAttributes().stateMachine);
          console.log(stateMachine);
        })
        .then(() => msg = stateMachine.dictate(handlerInput.requestEnvelope.request.intent.slots.NumeroTema.value) )
        .then(() => handlerInput.responseBuilder
        .speak(msg)
        .reprompt(msg)
        .getResponse());
