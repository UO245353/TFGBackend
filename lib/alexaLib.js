...
module.exports = app => {
  ...
  const handlers = {
    ...
    LeeTema: {
      canHandle(handlerInput) {
        console.log('LeeTema', JSON.stringify(handlerInput,0,2));
        return handlerInput.requestEnvelope.request.type === 'IntentRequest' && handlerInput.requestEnvelope.request.intent.name === 'LeeTema';
      },
      handle(handlerInput) {

        console.log('LeeTema 1', JSON.stringify(handlerInput,0,2));

        return app.promise.resolve()
        .then(() =>  handlerInput.responseBuilder
        .speak('Este es el tema ' + handlerInput.requestEnvelope.request.intent.slots.NumeroTema.value || handlerInput.requestEnvelope.request.intent.slots.TituloTema.value || '')
        .reprompt('Este es el tema')
