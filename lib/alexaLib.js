  ...
  const stateMachine = require('javascript-state-machine').factory({
    ...
    data: function(options) {
      ...
      return {
        ...
        responseNumber: options.responseNumber
      };
    },
    methods: {
  ...
  const handlers = {
    ...
    LeeTema: {
      ...
      handle(handlerInput) {
        ...
        .then(machine => (machine.can(alexaTransitions.DICTATE.name)) ? machine.dictate(themeNumber) : alexaResponses.IMPOSIBLE_ACTION)
        .catch(err => {
